...
public class UniverseJiho extends JPanel
{	private 		final Cell outermostCell;
	private final CellContainer cellContainer;
	private static	final UniverseJiho theInstance = new UniverseJiho();
	public final JPanel subPanel;
	...
	{	// Create the nested Cells that comprise the "universe." A bug
		...
		// on the screen.
		subPanel = new TestPanel(new ActionListener()
		{	public void actionPerformed(ActionEvent e)
		{	Clock.instance().startTicking(0);
			Storable previousCell = cellContainer.getPrevious();
			((TestPanel) subPanel).setTextField(cellContainer.getCurrentCount());
			if(previousCell==null){
				return;
			}
			outermostCell.transfer( previousCell, new Point(0,0), Cell.LOAD );
			refreshNow();
		}
		},new ActionListener()
		{	public void actionPerformed(ActionEvent e)
		...
		cellContainer = CellContainer.getInstance();
		...
		(	new Clock.Listener()
			{	public void tick()
				{
					cellContainer.storeCurrent(outermostCell);
					((TestPanel) subPanel).setTextField(cellContainer.getCurrentCount());
	...
	public static UniverseJiho instance()
	{	return theInstance;
	}
